...
if ($strExportErrorMessage == '') {
    ...
    if (is_array($YANDEX_EXPORT)) {
        ...
        foreach ($YANDEX_EXPORT as $ykey => $yvalue) {
            ...
            $filter = array("IBLOCK_ID" => $yvalue, "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_BRAND" => 58877,  '>CATALOG_STORE_AMOUNT_ID_617' => 1); // ID бренда Huberth
            ...
            while ($arAcc = $res->GetNext()) {
                ...
                $strTmpOff .= "<url>" . $usedProtocol . $arAcc['SERVER_NAME'] . htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]) . (strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '' : '&amp;') . "</url>\n";

                // Применение наценки в зависимости от цены
                if ($minPrice <= 5000) {
                    $newPrice = round($minPrice * 1.2); // Наценка 20%
                } else if ($minPrice <= 10000) {
                    $newPrice = round($minPrice * 1.1); // Наценка 10%
                } else {
                    $newPrice = $minPrice; // Нет наценки
                }

                // Генерация YML-данных для товара с обновленной ценой

                $strTmpOff .= "<price>" . $newPrice . "</price>\n";
